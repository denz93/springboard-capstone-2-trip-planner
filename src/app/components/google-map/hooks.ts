...
export function useGoogleMap<T extends HTMLElement>({ initialLocation, defaultZoom = 12 }: { initialLocation?: google.maps.LatLngLiteral | google.maps.LatLng, defaultZoom?: number } = {}) {
    const NEW_YORK = {
        lng: -74.005974,
        lat: 40.712776
    }
...
export function useGoogleLibrary<TName extends keyof typeof Libraries>(libName: TName) {
...
export function useGooglePlaceService() {
    ...
    const [placesService, setPlacesService] = useState<google.maps.places.PlacesService | null>(PlacesServiceInstance)
