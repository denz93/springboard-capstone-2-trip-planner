...
export default function TripStat({ initialTrip }: { initialTrip: Trip }) {
    const { data: trip } = trpc.trip.getTrip.useQuery({ id: initialTrip?.id }, { initialData: initialTrip })

    if (!trip) {
        return <div></div>
    }

    return <div className="stats stats-vertical lg:stats-horizontal ">
        {trip.hasChildren && <div className="stat place-items-center lg:px-10 gap-y-2">
        ...
        <div className="stat place-items-center lg:px-10 gap-y-4">
            <div className="stat-title text-2xl"><FaRegCalendarAlt /></div>
        ...
        <div className="stat place-items-center lg:px-10 gap-y-2">
            <div className="stat-title text-2xl"><IoIosToday /></div>
            <div className="stat-value  text-2xl">{(trip.startDate && trip.endDate) && differenceInDays(trip.endDate, trip.startDate) + ' days'}</div>
        ...
        <div className="stat place-items-center lg:px-10 gap-y-4">
            ...
            <div className="stat-value text-2xl">{getStatusBadge(trip.startDate, trip.endDate)}</div>
