...
export async function initData() {
  ...
  let trips = [
    ...
    await generateTrip(users[0], "Seattle", 1, true),
    await generateTrip(users[0], "San Jose", 5),
    await generateTrip(users[0], "New York City", 4, true),

    await generateTrip(users[1], "Las Vegas", 3, true),
    await generateTrip(users[1], "Los Angeles", 4),
    await generateTrip(users[1], "Boston", 5, true)
  ];
...
async function generateItinerary(
  ...
  // @ts-ignore
  const itinerary: z.infer<typeof SelectItinerarySchema> & {
    stops: z.infer<typeof SelectItineraryStopSchema>[];
  } = (
    await db
      .insert(Itinerary)
      .values({
  ...
  const stopPlaces = await db
    .insert(Place)
    .values(
      _.uniqBy(nearbyStops, (e) => JSON.stringify(e.geometry?.location))
        .slice(0, numOfStops)
        .map((s) => ({
