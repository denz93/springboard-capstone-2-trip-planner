import {
  ...
  InvalidTokenError,
  UserNotFoundError
} from "@/server/modules/auth/auth.error";
...
export async function verifyAccessToken(token: string) {
  try {
    ...
    if (err instanceof Error) console.debug(err.message);
...
export async function authenticateLocal(
  input: z.infer<typeof LocalLoginInputSchema>
) {
  ...
  if (!user) throw new UserNotFoundError("Email or password is not correct");
  const isMatch = await bcrypt.compare(password, user.account?.secret || "");
  if (!isMatch) throw new UserNotFoundError("Email or password is not correct");

  const userData = db.query.User.findFirst({ where: eq(User.id, user.id) });
  return userData;
}

export async function createAccountLocal(
  input: z.infer<typeof LocalRegisterInputSchema>
) {
  try {
...
export async function findUserAccount(
  userId: number,
  provider: NonNullable<z.infer<typeof SelectAccountSchema>["type"]>
) {
  return await db.query.Account.findFirst({
    where: and(eq(Account.userId, userId), eq(Account.type, provider))
  });
}

export async function verifyUserAccount(
  input: NonNullable<
    Pick<z.infer<typeof SelectAccountSchema>, "secret" | "type">
  >
) {
  const account = await db.query.Account.findFirst({
    where: and(
      eq(Account.secret, input.secret ?? ""),
      eq(Account.type, input.type)
    ),
    with: { user: true }
  });
