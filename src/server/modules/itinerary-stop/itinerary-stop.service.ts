...
export async function findByItineraryId(itineraryId: NonNullable<z.infer<typeof InsertItineraryStopSchema>["itineraryId"]>) {
  return (await db.select().from(ItineraryStop).where(eq(ItineraryStop.itineraryId, itineraryId)))
}